...
- RJ45 已连通磁性件
- 上位机始终不上链
- 原理图里 strap 阻值对，但复位释放和时钟稳定时间关系不清楚
- RGMII 延迟既在 PHY 配置里写了，又在板上预留了补偿

正式 review 时，合理结论应是：

- `ERROR`: strap 采样窗口不明确，不应放行。
- `ERROR`: RGMII 延迟策略不唯一，不应放行。
- `WARNING`: 线侧磁性件和 Bob-Smith 若未按参考设计闭环，后续 bring-up 风险高。

这种情况不能因为 `MDIO` 通了就误判为“硬件基本没问题”。
