...
- 一颗 PowerPAD 器件功耗较高
- 设计上在周围铺了大面积铜
- 但 exposed pad 下的热过孔数量少且位置偏外
- 过孔未定义塞孔/盖孔策略
- 扩热铜同时承担了高噪声功率回流

正式 review 时，应给出：

- `ERROR`: 热焊盘到主散热铜面的热路径不成立，不应放行。
- `WARNING`: 过孔工艺未定义，焊接一致性风险高。
- `WARNING`: 扩热铜兼作高噪声回流路径，会把热和噪声问题耦合在一起。

如果 review 只说“后续看热仿真”，而没有把热路径和工艺一起定下来，通常还没抓住关键点。
