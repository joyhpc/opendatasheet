...
- FPGA 设计含 DDR 和高速串行口
- 电源树里有 `VCCINT`、`VCCAUX`、`VCCIO`
- 收发器模拟 rail 与普通 IO rail 被合并处理
- 功耗预算只来自典型场景
- BGA 周边实际可放去耦空间没有评估

正式 review 时，至少应给出：

- `ERROR`: 收发器模拟 rail 与普通 IO rail 混处理，不应放行。
- `WARNING`: 功耗预算只用典型值，不能支持 rail 定型。
- `WARNING`: 若 BGA 周边去耦落点未验证，PDN 方案仍属纸面方案。

在这类例子里，最大风险通常不是“某颗 regulator 型号错”，而是 rail 分类和约束模型从一开始就错。
